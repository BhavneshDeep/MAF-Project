...
import { Loader2, Plus, Trash2, Edit3, Calendar, Sparkles, X, MapPin, Tag } from 'lucide-react';
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (isEditing && editingId) {
        const updated = await api.updateEvent(editingId, formData);
          ...
          {filteredEvents.length === 0 ? (
            ...
            <div className="grid grid-cols-1 gap-6">
              {filteredEvents.map(event => (
                <div
                  key={event.id}
                  className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden flex flex-col sm:flex-row group hover:border-rose-200 hover:shadow-md transition-all duration-300"
                >
                  {/* Thumbnail Cover */}
